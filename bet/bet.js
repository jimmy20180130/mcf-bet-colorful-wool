            ...
            let bet_result = new Promise(resolve => {
                bot._client.on('entity_metadata', async (entity) => {
                    try {
                        ...
                        if (item_id >= 180 && item_id <= 195) resolve(item_id)
                    } catch (e) {
...
async function process_bet_result(bot, wool, amount, player_id, type) {
    ...
    const odds = config.bet[`${type.slice(0, 1)}wool`][mcData.items[wool].name]

    if (Math.floor(new Decimal(amount).mul(new Decimal(odds)).toNumber()) > 0) {
        if (type == 'emerald') {
            const pay_result = await pay_handler(bot, player_id, Math.floor(new Decimal(amount).mul(new Decimal(odds)).toNumber()), 'e')
            ...
            await chat(bot, `/cointrans ${player_id} ${Math.floor(new Decimal(amount).mul(odds).toNumber())}`)
            await chat(bot, player_id)
